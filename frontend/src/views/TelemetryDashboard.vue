...
<div class="telemetry">
 ...
 <el-dialog v-model="selector" title="选择监测数据源" width="min(680px,94vw)" :close-on-click-modal="false">
  ...
  <el-form label-position="top"><el-form-item v-if="choice==='replay'" label="选择一次仿真或离线上传的数据"><el-select v-model="datasetId" filterable><el-option v-for="d in datasets" :key="d.id" :value="d.id" :label="`${d.name} / ${d.source==='simulation'?'仿真':'离线'} / ${d.fault_type||'未标注'}`"/></el-select></el-form-item>
   <template v-else><el-form-item label="采集会话"><el-select v-model="sessionId" filterable><el-option v-for="s in sessions" :key="s.id" :value="s.id" :label="`${s.name} / ${labels[s.state]} / ${s.sample_count} 条`"/></el-select></el-form-item><el-form-item v-if="admin" label="新建本次采集"><el-input v-model="sessionName" maxlength="200" placeholder="本次仿真/试验名称"/><el-button :loading="creating" :disabled="!sessionName.trim()" @click="createSession">创建采集会话</el-button></el-form-item></template>
...
async function choices(){datasets.value=await api('/api/v1/datasets/');sessions.value=await api('/api/v1/phm/telemetry/sessions/')}
...
<style scoped>
.telemetry{max-width:1400px;min-width:0}header,.toolbar,.chart-title{display:flex;align-items:center;gap:12px;flex-wrap:wrap}header,.chart-title{justify-content:space-between}.toolbar{padding:16px 0;border-bottom:1px solid #dde4ed}.speed{width:120px}.metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:20px;margin:24px 0}.metrics small,.metrics strong{display:block;overflow-wrap:anywhere}.metrics strong{font-size:24px;margin-top:8px}.chart-title .el-select{width:260px}.el-form{margin-top:24px}.el-form .el-select{width:100%}.el-alert{margin:12px 0}pre{white-space:pre-wrap;overflow-wrap:anywhere;padding:16px;background:#edf2f7}p{color:#627080;line-height:1.7}@media(max-width:700px){.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}
</style>
