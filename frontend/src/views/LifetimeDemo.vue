...
<h2>寿命预测报告</h2>
<div class="controls"><el-select v-model="id" filterable placeholder="选择包含 HI 的 CSV" @change="clear"><el-option v-for="d in datasets" :key="d.id" :label="d.name" :value="d.id"/></el-select><label>单机模型 <el-select v-model="componentId" @change="clear"><el-option v-for="x in modelIds" :key="x" :label="`部件 ${x}`" :value="x"/></el-select></label><el-button type="primary" :disabled="!id" :loading="loading" @click="run">运行训练模型</el-button></div>
<el-alert title="当前算法：项目组 health-main 剩余寿命集成模型" description="使用 CNN、BiRNN、BiLSTM、BiGRU、SRNN 五个时序模型，并由 RF/Ada 集成输出 RUL。数据集需包含至少 51 行 HI。" type="success" :closable="false"/>
...
<div class="metrics"><div><small>集成剩余寿命（RUL）</small><strong>{{Number(result.rul_value).toFixed(3)}}</strong><span>{{unitLabels[result.unit]||result.unit}}</span></div><div><small>当前健康指数（HI）</small><strong>{{Number(result.current_hi).toFixed(4)}}</strong><span>0–1健康指数</span></div><div><small>单机模型</small><strong>{{componentId}}</strong><span>共 {{result.sample_count}} 个样本</span></div><div><small>序列窗口</small><strong>{{result.sequence_length}}</strong><span>训练配置</span></div></div>
<h3>五模型及集成剩余寿命（RUL） 输出</h3><ReportChart :option="chart"/>
<h3>预测依据与限制</h3><el-descriptions border :column="2"><el-descriptions-item label="输入数据集">{{reportName}}</el-descriptions-item><el-descriptions-item label="有效样本数">{{result.hi_sequence.length}}</el-descriptions-item><el-descriptions-item label="算法">{{result.algorithm}}</el-descriptions-item><el-descriptions-item label="单机模型编号">{{componentId}}</el-descriptions-item><el-descriptions-item label="集成方法">RF 与 AdaBoost</el-descriptions-item><el-descriptions-item label="置信区间">训练资产未提供校准区间</el-descriptions-item></el-descriptions>
...
const unitLabels={sample_interval:'样本间隔',samples:'样本间隔',cycle:'循环次数',cycles:'循环次数'},datasets=ref([]),id=ref(),result=ref(null),error=ref(''),loading=ref(false),reportName=ref(''),componentId=ref('14'),modelIds=Array.from({length:15},(_,i)=>String(i+1))
const chart=computed(()=>{const r=result.value;if(!r)return {};const names=['CNN','BiRNN','BiLSTM','BiGRU','SRNN'];return {tooltip:{trigger:'axis'},legend:{data:[...names,'RF','Ada']},grid:{containLabel:true,left:45,right:40,bottom:75},xAxis:{type:'value',name:'预测窗口'},yAxis:{type:'value',name:'剩余寿命',scale:true},dataZoom:[{type:'inside'},{type:'slider'}],series:[...names.map(n=>({name:n,type:'line',showSymbol:false,data:(r.model_predictions[n]||[]).map((v,i)=>[i,v])})),{name:'RF',type:'line',showSymbol:false,lineStyle:{width:3},data:r.ensemble_rul.RF.map((v,i)=>[i,v])},{name:'Ada',type:'line',showSymbol:false,lineStyle:{width:3,type:'dashed'},data:r.ensemble_rul.Ada.map((v,i)=>[i,v])}]}})
