...
def get_model_params(cmg_type: str, target_cols: list):
    ...
    model_path_nms = BASE_DIR / "strategy2" / "models" / normalized_type / "isof_model.pkl"
    model_path_nm = BASE_DIR / "strategy2" / "models" / base_type / "isof_model.pkl"
    ...
    if model_path_nms.exists():
        ...
        raise FileNotFoundError(f"鎵句笉鍒癈MG鍨嬪彿 '{cmg_type}' 鐨剆trategy2妯″瀷鏂囦欢")
    
    MODEL_PATH = BASE_DIR / "strategy2" / "models" / folder_name / "isof_model.pkl"
    SCALERS_PATH = BASE_DIR / "strategy2" / "scalers" / folder_name / "scalers.pkl"
    SVD_SCALER_PATH = BASE_DIR / "strategy2" / "scalers" / folder_name / "svd_scaler.pkl"
    # 鍔犺浇褰掍竴鍖栧櫒
    ...
    params = {
        ...
        "anomaly_threshold": model_pkl.get("anomaly_threshold", None),
        "windows_config": model_pkl.get("windows_config", None),
    }
    return params
...
def train_scalers(df, target_cols, save_path=None):
    ...
    if save_path is None:
        save_path = BASE_DIR / "strategy2" / "scalers" / f"scalers_{time.strftime('%Y%m%d_%H%M%S')}.pkl"
...
def train_svd_features_scaler(svd_features_array, save_path=None):
    """璁粌SVD鐗瑰緛缂╂斁鍣?""
    scaler = StandardScaler()
    scaler.fit(svd_features_array)
    if save_path is None:
        save_path = BASE_DIR / "strategy2" / "scalers" / f"svd_scaler_{time.strftime('%Y%m%d_%H%M%S')}.pkl"
    if not save_path.parent.exists():
        save_path.parent.mkdir(parents=True, exist_ok=True)
    with open(save_path, "wb") as f:
...
def create_windows(data, window_size=64, step=32):
    """
    灏嗘暟鎹寜婊戝姩绐楀彛閲嶇粍 shape: [num_windows, window_size, feature_dim]
    """
...
def create_multi_scale_windows(data, window_sizes=[50, 100, 200], step=1):
    """
    鍒涘缓澶氬昂搴︾獥鍙?
    鍙傛暟:
        data: 杈撳叆鏁版嵁锛宻hape=(n_samples, n_features)
        window_sizes: 绐楀彛澶у皬鍒楄〃锛屽[50, 100, 200]
        step: 婊戝姩姝ラ暱
    杩斿洖:
        windows_dict: 瀛楀吀锛宬ey涓虹獥鍙ｅぇ灏忥紝value涓虹獥鍙ｅ垪琛?
    """
    ...
    for win_size in window_sizes:
        if win_size > n_samples:
            print(f"璀﹀憡: 绐楀彛澶у皬{win_size}澶т簬鏁版嵁闀垮害{n_samples}锛岃烦杩囨绐楀彛")
            continue
        
        windows = []
        for i in range(0, n_samples - win_size + 1, step):
            window = data[i:i + win_size]
            windows.append(window)
        
        windows_dict[win_size] = windows
...
def svd_feature_extract(mat, target_cols, scalers):
    """
    SVD鍒嗚В鎻愬彇鐗瑰緛
    鍙傛暟:
        mat: 绐楀彛鏁版嵁锛宻hape=(window_size, n_features)
        target_cols: 鐩爣鍒楀悕
        scalers: 鏁版嵁鏍囧噯鍖栧櫒
    杩斿洖:
        svd_features: 濂囧紓鍊肩壒寰佸悜閲?
    """
...
def extract_multi_scale_svd_features(windows_dict, target_cols, scalers):
    """
    浠庡灏哄害绐楀彛涓彁鍙朣VD鐗瑰緛
    鍙傛暟:
        windows_dict: 鐢眂reate_multi_scale_windows鍒涘缓鐨勫瓧鍏?
        target_cols: 鐩爣鍒楀悕
        scalers: 鏁版嵁鏍囧噯鍖栧櫒
    杩斿洖:
        features_list: 澶氬昂搴VD鐗瑰緛鍒楄〃
    """
    features_list = []
    
    for win_size, windows in windows_dict.items():
        print(f"鎻愬彇绐楀彛澶у皬{win_size}鐨凷VD鐗瑰緛锛屽叡{len(windows)}涓獥鍙?)
        for window in tqdm(windows):
            # 鎻愬彇璇ョ獥鍙ｇ殑SVD鐗瑰緛
            svd_features = svd_feature_extract(window, target_cols, scalers)
            features_list.append(svd_features)
    
    return np.array(features_list)

def extract_svd_features(windows, target_cols, scalers):
    """
    浠庡崟涓€绐楀彛澶у皬鐨勬暟鎹腑鎻愬彇SVD鐗瑰緛
    鍙傛暟:
        windows: 绐楀彛鏁版嵁鍒楄〃锛宻hape=(n_windows, window_size, n_features)
        target_cols: 鐩爣鍒楀悕
        scalers: 鏁版嵁鏍囧噯鍖栧櫒
    杩斿洖:
        features_list: SVD鐗瑰緛鍒楄〃
    """
    features_list = []
...
class DynamicThreshold:
    """鍔ㄦ€侀槇鍊艰绠楃被"""
    def __init__(self, window_size=100, percentile=99):
        self.window_size = window_size
    ...
    def update(self, value):
        ...
        # 褰撶獥鍙ｆ湭婊℃椂浣跨敤鎵€鏈夊彲鐢ㄥ€?
        current_threshold = np.percentile(list(self.values), self.percentile)
