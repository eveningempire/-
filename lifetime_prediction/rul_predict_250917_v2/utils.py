...
# PHM鍨嬪彿閰嶇疆鏄犲皠琛紙鍩虹閰嶇疆锛屽彧鍖呭惈NM鏍煎紡锛?
...
for base_type, config in _PHM_TYPE_COL_CONFIG.items():
    ...
    # 娣诲姞NMS鏍煎紡锛堝皢"NM"鏇挎崲涓?NMS"锛?
    nms_type = base_type.replace("NM", "NMS")
...
def fuzzy_match_cmg_type(cmg_type: str) -> str:
    ...
    raise ValueError(
        ...
        f"璇风‘淇濆瀷鍙峰悕绉颁腑鍖呭惈浠ヤ笅妯″紡涔嬩竴锛?NM銆?NM銆?5NM銆?00NM銆?000NM锛堟垨瀵瑰簲鐨凬MS鐗堟湰锛?
    )


def normalize_cmg_type(cmg_type: str) -> str:
    """
    鏍囧噯鍖朇MG鍨嬪彿鍚嶇О锛屾敮鎸佹ā绯婂尮閰嶅苟缁熶竴杞崲涓烘爣鍑嗘牸寮?
    
    璇ュ嚱鏁扮幇鍦ㄤ娇鐢╢uzzy_match_cmg_type杩涜妯＄硦鍖归厤锛屾敮鎸侊細
    - 鐩存帴鍖归厤锛氬 "500NM", "500nm", "500NMS"
    - 妯＄硦鍖归厤锛氬 "TG-15NMS", "JB-500NM-V2"
...
def get_model_params(cmg_type: str, target_cols: list):
    """
    鑾峰彇妯″瀷鍙傛暟锛屾敮鎸丯M鍜孨MS涓ょ鏍煎紡鐨凜MG鍨嬪彿
    
    Args:
        cmg_type: PHM鍨嬪彿锛堟敮鎸丯M鎴朜MS鏍煎紡锛?
        target_cols: 鐩爣鍒楀悕鍒楄〃
    ...
    for candidate in [normalized_type, normalized_type.replace("NMS", "NM")]:
        candidate_path = BASE_DIR / "models" / candidate / "ae_model.pkl"
    ...
    if folder_name is None:
        raise FileNotFoundError(
            f"鎵句笉鍒癈MG鍨嬪彿 {cmg_type} 鐨勬ā鍨嬫枃浠躲€?
            f"宸插皾璇曡矾寰? {BASE_DIR / 'models' / normalized_type} 鍜?"
            f"{BASE_DIR / 'models' / normalized_type.replace('NMS', 'NM')}"
        )
    
    MODEL_PATH = BASE_DIR / "models" / folder_name / "ae_model.pkl"
    SCALERS_PATH = BASE_DIR / "scalers" / folder_name / "scalers.pkl"
    SIGMA_SCALER_PATH = BASE_DIR / "scalers" / folder_name / "sigma_scaler.pkl"
