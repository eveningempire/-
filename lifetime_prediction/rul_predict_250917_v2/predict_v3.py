...
class Predictor:
    ...
    def process_datadf(cmg_type, cmg_target_cols, data_df, window_size=64, step=64):
        ...
        # 浣跨敤妯＄硦鍖归厤鍚庣殑PHM鍨嬪彿
        try:
            matched_cmg_type = utils.fuzzy_match_cmg_type(cmg_type)
        except ValueError:
            # 濡傛灉妯＄硦鍖归厤澶辫触锛屼娇鐢ㄥ師濮嬬被鍨嬶紙鍚戝悗鍏煎锛?
            matched_cmg_type = cmg_type
        model_params = utils.get_model_params(matched_cmg_type, target_cols=target_cols)
        data = data_df[target_cols].values
        data_windows = utils.create_windows(data, window_size=window_size, step=step)
        ae = model_params["model"]
        sigmas = []
        for dw in data_windows:
            sigma = utils.svd_feature_extract(
                dw, model_params["target_cols"], model_params["scalers"], model_params["sigma_scaler"]
            )
            sigmas.append(sigma)
        sigmas_arr = np.array(sigmas)
        batch_tensor = torch.from_numpy(sigmas_arr).float()
        ae.eval()
        with torch.no_grad():
            reconstructed, z = ae(batch_tensor)
        errs = utils.rmse(batch_tensor, reconstructed, dim=1)
        return errs, model_params

    @staticmethod
    def get_windowed_hi(arr, p_mean, p_std, window_size=128, step=1, h=0.5, l=0.0):
        """璁＄畻绐楀彛鍖栫殑鍋ュ悍鎸囨暟 HI"""
        arr = np.asarray(arr)
        n = arr.shape[0]
        if window_size > n:
    ...
    def calc_rul_weighted(
        ...
        timestamp_start = utils.parse_timestamp_flexible(time_stamps.iloc[0])
        timestamp_start_use = utils.parse_timestamp_flexible(time_start_use)
        timestamp_end = utils.parse_timestamp_flexible(time_stamps.iloc[-1])
        ...
        # if hi[0] <= hi[-1]:
        #     hi_high = hi_low = np.ones(len(hi)) * hi[-1]
        #     return rul_thretical, hi_high, hi_low
    ...
    def predict_batch(
        ...
        try:
            matched_cmg_type = utils.fuzzy_match_cmg_type(cmg_type)
            print(f"PHM鍨嬪彿妯＄硦鍖归厤: '{cmg_type}' -> '{matched_cmg_type}'")
        except ValueError as e:
            raise ValueError(f"鏃犳硶璇嗗埆PHM鍨嬪彿 '{cmg_type}': {str(e)}")
        
        cmg_target_cols = utils.PHM_TYPE_COL_NAME_MAP.get(matched_cmg_type, None)
        if cmg_target_cols is None:
            raise ValueError(f"涓嶆敮鎸佺殑cmg_type: {matched_cmg_type}锛堜粠 '{cmg_type}' 鍖归厤寰楀埌锛夛紝璇锋鏌ワ紒")
        # 棰勪及骞惰嚜閫傚簲璋冩暣step
        ...
        # ws = max(len(all_res) // 100, 4)
        ws = len(all_res) // 100
        if use_median:
            p_mean = model_params["recon_error_median"]
            p_std = model_params["recon_error_mad"] * 1.4826
        else:
            p_mean = model_params["recon_error_mean"]
            p_std = model_params["recon_error_std"]
        hi_ori = cls.get_windowed_hi(all_res, p_mean, p_std, window_size=ws, step=ws, h=h, l=l)
