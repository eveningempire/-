...
def _getTfIdfModel(txts, ngrams=3, p_value_limit=0.95):
    ...
    vectorizer = feature_extraction.text.TfidfVectorizer(max_features=max(5000, (len(set(labels))*0.98)), ngram_range=(1, ngrams))
...
def read_fmeca(df, check_invalid_col=["单元名称、型号或图号（2）", "故障模式(4)"],
    ...
    for component_value, faultmode_value, detection_value, \
        ...
        if not faultmode_value.replace(" ", "").replace("-", ""):
            continue
        for detection_value_ in detection_value.replace("\n",";").replace("、",";").replace("；",";").split(";"):
            if not detection_value_:
                continue
            if faultmode_value not in detection_value_ or _check_repeat("".join(detection_value_.split(faultmode_value, 1)), d_col_sign):
                result_list['nodes'].append({'text': detection_value_, 'type': ckptnode})
                result_list['edges'].append({'from': faultmode_value, 'to': detection_value_})
        ...
        if faultcause_values:
            for faultcause_value in faultcause_values.split(";"):
                ...
                if faultcause_value not in df.loc[:,'faultmode_value'].values.flatten():
                    if faultcause_value not in raw_cause2sys.keys():
                        raw_cause2sys[faultcause_value] = component_value
                    else:
                        raw_cause2sys[faultcause_value] = None
        if self_effect_values:
            for self_effect_value in self_effect_values.split(";"):
                ...
                if faultcause_value not in df.loc[:,'faultmode_value'].values.flatten():
                    if self_effect_value not in raw_selfeffect2sys.keys():
                        raw_selfeffect2sys[self_effect_value] = component_value
                    else:
                        raw_selfeffect2sys[self_effect_value] = None
            ...
            for final_effect_value in final_effect_values.split(";"):
                ...
                if final_effect_value not in node_registre:
                    result_list['nodes'].append({'text': final_effect_value, 'type': resnode})
                    node_registre.add(final_effect_value)
    validNodeTexts = [text for edge in result_list['edges'] for text in [edge["from"], edge["to"]]]
    result_list['nodes'] = [node for node in result_list['nodes'] if node["text"] in validNodeTexts]
    nodeNames = [node["text"] for node in result_list['nodes']]
    result_list['nodes'] = [node for nodeId, node in enumerate(result_list['nodes'] )if node["text"] not in nodeNames[:nodeId]]
    nodeNames = [node["text"] for node in result_list['nodes']]
    ...
    for k in raw_cause2sys.keys():
        raw_selfeffect2sys[k] = None
    raw_cause2sys = {k: v for k, v in raw_cause2sys.items() if v}
    raw_selfeffect2sys = {k: v for k, v in raw_selfeffect2sys.items() if v}

    subsystems = {subsysid: {"name": subsysInfo[0], "nodesId": sorted(
        set([nodeNames.index(noden) for noden in subsysInfo[1]] + [nodeNames.index(cause) for cause, sys in raw_cause2sys.items() if sys==subsysInfo[0]] + [nodeNames.index(selfeffect) for selfeffect, sys in raw_selfeffect2sys.items() if sys==subsysInfo[0]])
        )} for subsysid, subsysInfo in enumerate(subsystem.items())} 
