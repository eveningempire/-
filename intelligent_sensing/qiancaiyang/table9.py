    ...
    if results:
        ...
        for key in metric_keys:
            ...
            for m in methods:
                ...
                results[m]["metrics_norm"] = results[m].get("metrics_norm", {})
                results[m]["metrics_norm"][key] = results[m]["metrics"][key] / min_val

        # 使用归一化后的结果计算综合指标
        for m in methods:
            normalized_dtw_time = results[m]["metrics_norm"]["dtw_time"]
            normalized_dtw_freq = results[m]["metrics_norm"]["dtw_frequency"]
...
def get_fast_slow_reconstruction_res(segments=None, start_time_str=None):
    ...
    # 3. 构造返回结构 (Key 使用英文，对接 test_real_data_1hz 格式)
    methods_map = {"linear": y_linear, "cubic": y_cubic, "freq": y_guided}
    results = {}
    ...
    mag_ref = np.abs(np.fft.fft(y_ref))[pos_mask]

    all_metrics = {}
    for name, y_rec in methods_map.items():
        metrics = metrics_compute(y_ref, y_rec, fs_ds=None, fs_ori=1.0)
        all_metrics[name] = metrics
        mag_rec = np.abs(np.fft.fft(y_rec))[pos_mask]
        rmse_val = float(np.sqrt(np.mean((y_ref - y_rec)**2)))
        ...
        results[name] = {
            "time_domain": {
                ...
                "x3": t_ref,
                "y3": y_ref,
                "similarity": float(metrics['dtw_time']),
            },
            "freq_domain": {
                "x": {"value": freq_axis, "label": "频率 (Hz)"},
                "y1": {"value": mag_ref, "label": "原始信号频谱"},
                "y2": {"value": mag_rec, "label": f"{name}重建信号频谱"},
                "similarity": float(metrics['dtw_frequency']),
            },
            "metrics": {
                "RMSE": rmse_val,
                "DTW": float(metrics['dtw_time'])
            },
            # "combined_similarity": (float(metrics['dtw_time']) + float(metrics['dtw_frequency'])) / 2,
        }
    # 获取所有方法名
    methods = list(all_metrics.keys())

    # 获取所有metric的键名
    metric_keys = list(all_metrics[methods[0]].keys())
    ...
    for key in metric_keys:
        ...
        # 归一化：除以最小值
        for m in methods:
            if "metrics_norm" not in all_metrics[m]:
                all_metrics[m]["metrics_norm"] = {}
            all_metrics[m]["metrics_norm"][key] = all_metrics[m][key] / min_val

    # 使用归一化后的结果计算综合指标
    for m in methods:
        normalized_dtw_time = all_metrics[m]["metrics_norm"]["dtw_time"]
        normalized_dtw_freq = all_metrics[m]["metrics_norm"]["dtw_frequency"]
        results[m]["combined_similarity"] = (normalized_dtw_time + normalized_dtw_freq) / 2
        print(f"Method: {m}, Combined Similarity: {results[m]['combined_similarity']:.4f}, DTW Time Norm: {normalized_dtw_time:.4f}, DTW Freq Norm: {normalized_dtw_freq:.4f}, DTW Time: {all_metrics[m]['dtw_time']:.4f}, DTW Freq: {all_metrics[m]['dtw_frequency']:.4f}")
    # # === 新增：复用归一化和综合指标计算逻辑 ===
    # methods = list(results.keys())
    # if results:
    #     metric_keys = ["dtw_time", "dtw_frequency"]
    #     for key in metric_keys:
    #         breakpoint()
    #         values = [results[m]["metrics"][key] for m in methods]
    #         min_val = min(values) if min(values) > 0 else 1e-9
    #         for m in methods:
    #             results[m]["metrics_norm"] = results[m].get("metrics_norm", {})
    #             results[m]["metrics_norm"][key] = results[m]["metrics"][key] / min_val
        
    #     for m in methods:
    #         norm_time = results[m]["metrics_norm"]["dtw_time"]
    #         norm_freq = results[m]["metrics_norm"]["dtw_frequency"]
    #         # 更新前端使用的综合指标
    #         results[m]["combined_similarity"] = (norm_time + norm_freq) / 2
...
def get_intermittent_reconstruction_res(segments=None, start_time_str=None):
    ...
    # 3. 构造返回结构 (对接 test_real_data_1hz 格式)
    methods_map = {"linear": y_linear, "cubic": y_cubic, "freq": y_guided}
    results = {}
...
if __name__ == "__main__":
    ...
    plot_1hz_results(res_json_1hz, pct=0.30, method="freq", save=False, save_dir='figs/1Hz')
    ...
    # res = get_low_sampling_res(4)
    # plot_low_sampling_results(res)
    ...
    plot_1hz_results(res_json_fs, pct=0.30, method="freq", save=False)
    ...
    for method in res_dict_inter:
        ...
        print(f"[{method}] RMSE: {m['RMSE']:.4f}, DTW: {m['DTW']:.2f}")
    plot_1hz_results(res_json_inter, pct=0.30, method="freq", save=False)
