		...
		methods: {
			...
			$_dragNode(item) {
				...
				if (item.type == 'subsystem-node') {
				  // 1.创建新的的system
				  let new_system = {
					system_id: this.$props.gData.SystemData.at(-1).system_id + 1,
					name: "子系统"+(this.$props.gData.SystemData.at(-1).system_id + 1),
	...
	'ate-comp-control-panel',
	{
		...
		template: `
			<div>
				<el-button-group>
					...
					<el-button v-if="setDefault(translateRest, true)" type="plain" size="small" @click="$_translateRest">定位图形</el-button>
				...
				<el-upload v-if="setDefault(importFmeca, true)" style="display:inline-block; margin-left: -5px;"
					action="" :auto-upload="false" accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
				...
				<el-dialog width="60%" :title="'流图载入方式'" :visible.sync="importData_dialogVisible" :modal="false">
					...
					<div style="display:flex;flex-direction:column;justify-content:space-between;align-items:center;gap:10px">
						...
						<el-upload style="display:inline-block; margin-left: -5px;" action="" :auto-upload="false" accept=".vsdx"
						:multiple="false" :show-file-list="false" :on-change="$_importData_visio">
							<el-button type="primary" plain>载入全局数据(visio格式)</el-button>
						</el-upload>
						...
						:multiple="false" :show-file-list="false" :on-change="$_importData_global">
							<el-button type="primary" plain>载入全局数据(json格式)</el-button>
						</el-upload>
		...
		methods: {
			...
			$_translateRest () {
				...
				const graphData = this.$props.lf.getGraphData();
				// 提取节点信息
				...
				// 计算平均位置
				if (nodes.length !== 0) {
					const averageX = totalX / nodes.length;
					const averageY = totalY / nodes.length;
					this.$props.lf.focusOn({coordinate:{x:averageX,y:averageY}})
				}
			...
			$_importData_visio(file) {
				...
				axios.post('/multi-info-edit/upload-visio/', fd)
				.then((res) => {
	...
	'ate-multi-signal-flow-graph',
	{
		...
		template: `
			<div class="logic-flow-view">
				...
				<div class="model—tree">
					<el-tree :data="module_tree" :expand-on-click-node="false"
		...
		methods: {
			...
			handleUpdatImportData(data) {
				...
				if (data.type == 'global') {
					// 检查并修复导入数据的结构
					this.gData = this._fixImportDataStructure(data.value)
				    let root_id = this.gData.SystemData.find(item => item.parent_id == null).system_id
			...
			handleNodeClick(data) {
				// 检查数据是否有效
				if (!data || !data.id) {
					console.warn('handleNodeClick: 无效的data', data)
					return
				}
				...
				// 检查是否找到系统
				if (!current_system) {
					console.warn('handleNodeClick: 找不到系统', data.id, this.gData.SystemData)
					return
				}

				while (current_system != undefined) {
					this.current_system_breadcrumb.push({ name: current_system.name || `系统${current_system.system_id}`, id: current_system.system_id })
				...
				// 检查当前系统的数据是否有效
				let current_system_data = this.gData.SystemData.find(item => item.system_id == data.id)
				if (!current_system_data || !current_system_data.data) {
					console.warn('handleNodeClick: 当前系统数据无效', current_system_data)
					return
				}

				// 临时补丁，当子系统的属性如input或output发生变化时，先渲染一次，更新子系统的外观属性，手动重新调整 线的起点和终点位置 然后再渲染一次
				// 1. 第一次渲染 并获取渲染后的绘图数据
				this.lf.render(current_system_data.data)
				let rendered_data = this.lf.getGraphData()

				// 2. 调整与子系统相关的连线的起点和终点位置
				let subsystem_nodes = rendered_data.nodes.filter(item => item.type == 'subsystem-node')

				for (let node of subsystem_nodes) {
					for (let anchor of node.anchors) {
						if (anchor.type == 'left') {
							// 调整连线的终点位置
							let relative_edges = rendered_data.edges.filter(item => item.targetAnchorId == anchor.id)
							...
							// 调整连线的起点位置
							let relative_edges = rendered_data.edges.filter(item => item.sourceAnchorId == anchor.id)
							relative_edges.forEach(item => {
							item.pointsList = []
				...
				if (nodes.length != 0) {
					const averageX = totalX / nodes.length;
					const averageY = totalY / nodes.length;
					this.lf.focusOn({coordinate:{x:averageX,y:averageY}})
				}
			}
		},
	});
// 将msfgComp暴露为全局变量，以便在HTML页面中使用
window.msfgComp = msfgComp;