...
/**function changeTableColor1BeforeBackground(rgbColor, randomNum) {
  // 创建一个 <style> 元素
  const style = document.createElement('style');
  // 设置 <style> 元素的类型
  style.type = 'text/css';
  // 构造 CSS 规则
  const cssRule = `.table-node .table-color .table-color-${randomNum}::before { background: ${rgbColor}; }`;
  console.log(cssRule)
  ...
  if (style.styleSheet) {
    ...
    // 适用于 Chrome, Firefox 等
    style.appendChild(document.createTextNode(cssRule));
  }
  // 将 <style> 元素添加到 <head> 中，以应用样式
...
class SubsystemNode extends HtmlNode {
  ...
  setHtml (rootEl) {
    rootEl.innerHTML = ""
    const {
      properties: { fields, tableName, typeColor }
    } = this.props.model
    ...
    if (tableName.length <= 12){
      ...
      tableNameElement.innerText = tableName.slice(0, 10) + "..."
    }
    ...
    for (let i = 0; i < max_length; i++) {
      ...
      if ((fields.inputNames || []).length > i) {
        itemInput.innerText = fields.inputNames[i]; //IN "+(i+1)
      } else if (fields.input > i){
      ...
      if ((fields.outputNames || []).length > i) {
        itemOutput.innerText = fields.outputNames[i]; //OUT "+(i+1)
      } else if (fields.output > i){
...
class SubsystemNodeModel extends HtmlNodeModel {
  ...
  getDefaultAnchor () {
    ...
    // }
    /*if (this.anchors){
      let anc_ins = this.anchors.filter(itm=>itm.type=="left");
      let anc_outs = this.anchors.filter(itm=>itm.type=="right");
      anc_ins.forEach((itm, itmid)=>{
        anchors[itmid].id = itm.id
      })
      anc_outs.forEach((itm, itmid)=>{
        anchors[fields.input.length+itmid].id = itm.id
      })
    }*/
    return anchors
  }
