...
def get_predict_result(method="base", exp="015"):
    ...
    if method == "base":
        ...
        if exp == "015":
            test_data2, _ = get_test_data("018")  # 报告中mu，nu测试结果是用018得到的，埋了个大坑。。。
            with torch.no_grad():
        ...
        res_u_v = {
            "metrics": get_metrics(test_labels, predictions_denorm_u_v),
        ...
        res_b_k = {
            "metrics": get_metrics(test_labels, predictions_denorm_b_k),
            "true_labels": np.copy(test_labels).flatten(),
