#!/usr/bin/env python
"""
閫氱敤淇鑴氭湰锛氫慨澶嶆墍鏈夋ā鍨嬬殑閮ㄤ欢鎻愬彇闂
鍩轰簬JSON閰嶇疆鏂囦欢鎻愬彇閮ㄤ欢骞舵洿鏂癕SFG閰嶇疆
"""
...
def extract_components_from_json():
    ...
    try:
        # 1. 璇诲彇JSON閰嶇疆鏂囦欢
        json_file_path = r"C:\Users\megan\Desktop\cmg_v1.0\scripts\PHM_MSFG_refined.json"
...
def fix_model_components(model_name, components):
    """淇鎸囧畾妯″瀷鐨勯儴浠堕厤缃?""
    print(f"\n馃敡 淇 {model_name} 鐨勯儴浠堕厤缃?")
    print("-" * 60)
    ...
    try:
        ...
        if not active_msfg:
            print(f"鉂?娌℃湁鎵惧埌{model_name}鐨勬椿璺僊SFG閰嶇疆")
            return False
        
        print(f"鉁?鎵惧埌娲昏穬MSFG: {active_msfg.name} (ID: {active_msfg.id})")
        
        # 2. 鏇存柊MSFG鐨刢omponent_names瀛楁
        print(f"馃攧 鏇存柊MSFG鐨刢omponent_names瀛楁...")
        ...
        print(f"馃帀 {model_name} 淇瀹屾垚锛?)
        return True
        
    except PHMModel.DoesNotExist:
        print(f"鉂?鎵句笉鍒皗model_name}鐨凜MG妯″瀷")
        return False
    except Exception as e:
...
def main():
    """涓诲嚱鏁?""
    print("馃敡 閫氱敤淇鑴氭湰锛氫慨澶嶆墍鏈夋ā鍨嬬殑閮ㄤ欢鎻愬彇闂")
    print("=" * 80)
    
    # 1. 浠嶫SON鏂囦欢鎻愬彇閮ㄤ欢
    components = extract_components_from_json()

    if not components:
        print("鉂?鏃犳硶浠嶫SON鏂囦欢鎻愬彇閮ㄤ欢锛岄€€鍑?)
        return
    
    # 2. 瀹氫箟闇€瑕佷慨澶嶇殑妯″瀷鍒楄〃
    models_to_fix = ["Test"]  # 鍙互鏍规嵁闇€瑕佹坊鍔犳洿澶氭ā鍨?
    
    # 3. 淇姣忎釜妯″瀷
    success_count = 0
    for model_name in models_to_fix:
        try:
            if fix_model_components(model_name, components):
                success_count += 1
        except Exception as e:
            print(f"鉂?淇{model_name}鏃跺嚭閿? {e}")
    
    # 4. 鎬荤粨
    print(f"\n馃帀 淇鎬荤粨:")
    print(f"   鎴愬姛淇: {success_count}/{len(models_to_fix)} 涓ā鍨?)
    
    if success_count == len(models_to_fix):
        print("鉁?鎵€鏈夋ā鍨嬩慨澶嶆垚鍔燂紒")
        print("鐜板湪鍙互灏濊瘯鍦∕SFG閮ㄤ欢瀵瑰簲绠＄悊椤甸潰涓粦瀹氭槧灏勪簡銆?)
    else:
        print("鈿狅笍 閮ㄥ垎妯″瀷淇澶辫触锛岃妫€鏌ュ叿浣撻敊璇俊鎭€?)
