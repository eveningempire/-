...
**闂**锛?
- `str(item['data'])` 可能产生不同的字符串表示
- 浮点数精度问题：`1.0` vs `1.0000000000000001`
- 字典键顺序问题：`{'a':1, 'b':2}` vs `{'b':2, 'a':1}`

#### **问题3：数据库查询范围过大**
```python
PHMData.objects.filter(
    cmg=session.cmg, 
    timestamp__in=[item['timestamp'] for item in parsed_data]  # 16000个时间戳
)
```
...
2. **过度复杂的应用层过滤**：应该在数据库层处理
3. **性能问题**：大量数据查询和过滤导致性能下降
