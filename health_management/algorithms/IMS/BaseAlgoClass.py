...
# from ...dataBaseEngine import Session, DetectModelBase
...
class BaseModel:
    """故障检测算法的模板（基类）
    """
    def __init__(self, index=0, name="model", pnames=[], component=None, config={}):
    ...
    def save_model(self, count=0, raise_err=False):
        ...
        """
        try:
            print(f"{self.uuid}.json", 5555)
            config = self.to_json() #long-blob
            with open(f"{self.uuid}.json", "wb+") as f:
                f.write(config)
            print(f"{self.uuid}.json", 6666)
            """
            session = Session()
            session.merge(DetectModelBase(uuid=self.uuid, model_params=config))
            session.commit()
            session.close()
            """
    ...
    def load_model(self, uuid=None):
        ...
        if uuid is None:
            uuid = self.uuid
        session = Session()
        config = session.query(DetectModelBase.model_params).filter(DetectModelBase.uuid==uuid).first()
        if config:
    ...
    def singleValidate(self, dataFrames):
        ...
        """
        _, scoreList, _ = self.validate(dataFrames)
        addativeList, labelList = addativeLowList
