...
DATA_DIR = BASE_DIR / "year2/data"


def get_online_learning_res_bak(group: int = 1):
    """
    获取在线学习的结果数据（原始备份版本）。
...
if __name__ == "__main__":
    ...
    # 2. 测试新开发的实时自适应在线预测服务
    print("\n" + "=" * 60)
    print("测试新开发的实时自适应在线预测服务...")
    print("=" * 60)

    # 第一次运行会执行仿真并保存，后续运行将直接读取
    service_res_json = get_online_adaptive_results_service(force_run=False)
    ...
    print("\n服务函数返回的数据结构示例 (OSELM):")
    oselm_data = service_res[0]
    print(f"键名: {list(oselm_data.keys())}")
    print(f"数据量 (点数): {len(oselm_data['original_data'])}")
