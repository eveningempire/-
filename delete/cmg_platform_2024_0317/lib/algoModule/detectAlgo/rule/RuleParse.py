...
class ruleParse:
    ...
    def _func_convert(self, rule, seqs_=[]):
        ...
        for r_split in reversed(rule_split):
            ...
            if rule[a] in ["[", "<", "{"]: # Type1 - "[time]", "<time>", "{frame_all,frame_threshold}"
                ...
                if rule[a] == "{":
                    if len(condition) > 2:
                        self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入过多"})
                        self.warning += 1
                    elif len(condition) < 2:
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule[a:b], seqs)}]输入过少"})
                        self.error += 1
                    try:
                        secondframe1 = int(float(condition[0]))
                        if "." in condition[0]:
                            self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入1必须为整数"})
                            self.warning += 1
                    except Exception as e:
                        try:
                            secondframe1 = int(eval(condition[0]))
                            self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入1不能为表达式"})
                            self.warning += 1
                        except  Exception as e:
                            self.log.append({"type": "error", "text": f"[{self._backlog(rule[a:b], seqs)}]输入1格式有误"})
                            self.error += 1
                            secondframe1 = None
                    try:
                        secondframe2 = int(float(condition[1]))
                        if "." in condition[1]:
                            self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入2必须为整数"})
                            self.warning += 1
                    except Exception as e:
                        try:
                    ...
                    if (not secondframe1 is None) and (not secondframe2 is None) and secondframe1 < secondframe2:
                        ...
                        self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入1必须不小于输入2"})
                        self.warining += 1
                else:
                    if len(condition) > 1:
                        self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入过多"})
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.log.append({"type": "warn", "text": f"[{self._backlog(rule[a:b], seqs)}]输入不能为表达式"})
                            self.warning += 1
                        except  Exception as e:
                            self.log.append({"type": "error", "text": f"[{self._backlog(rule[a:b], seqs)}]输入格式有误"})
                            self.error += 1
                            secondframe1 = None
                ...
                if warned:
                    self.log.append({"type": "warn", "text": f"[{rule_origin}]表达式需用\"()\"包裹"})
                    self.warning += 1
                if left > 0: # lack some ), which means meanwhile i reaches end
                    self.log.append({"type": "warn", "text": f"[{rule_origin}]缺少{left}个\")\""})
                    self.warning += 1
                express = rule[b:i] + ")"*left
                result, typ = self._stringconvert(express, seqs)
                if typ not in ["Unknown", "bool"]:
                    self.log.append({"type": "error", "text": f"[{rule_origin}]输出应为布尔值"})
                    self.error += 1
                ...
                if left > 0:
                    self.log.append({"type": "warn", "text": f"[{rule_origin}]缺少{left-1}个\")\""})
                    self.warning += 1
                ...
                if func_name in self.funcDict.keys():
                    ...
                    if result is None:
                        ...
                        if func_name in self.seqs.keys():
                            if express_origin in self.seqs[func_name].keys():
                                self.seqs[func_name][express_origin] = (self.seqs[func_name][express_origin][0], \
                                                                        self._refresh_time_count(self.seqs[func_name][express_origin][1], timeCount))
                            else:
                    ...
                    self.log.append({"type": "error", "text": f"{rule_origin}函词{func_name}未定义"})
                    self.error += 1
                    if args[0].replace(" ",""):
        ...
        if "[" in rule or "{" in rule or "]" in rule or "}" in rule:
            self.log.append({"type": "warn", "text": f"{rule_} 中存在非函词的特殊括号"})
            self.warning += 1
    ...
    def _convert_args(self, args, args_need, multinput, rule_origin, seqs_, func_name):
        ...
        if len(args) > len(args_need):
            if not multinput:
                self.log.append({"type": "warn", "text": f"[{rule_origin}]输入过多"})
                self.warning += 1
            else:
        ...
        for func_pos, typin, typwant, arg_default in args_need:
            if func_pos >= len(args):
                if arg_default is None:
                    self.log.append({"type": "error", "text": f"[{rule_origin}]缺少输入{func_pos}"})
                    self.error += 1
                ...
                if not args[func_pos].replace(" ","") or args[func_pos].replace(" ","") in self.videArgSymb:
                    ...
                    if typreal not in ["Unknown", typwant]:
                        self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}为{typreal}类型(应为{typwant}类型)"})
                        self.error += 1
                    else:
                ...
                if typwant == "istime":
                    try:
                        ...
                        if istime not in [0, 1]:
                            self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}(istime类型)必须为0/1"})
                            self.warning += 1
                        time_frame = "time" if istime else "frame"
                        args_[func_pos] = 1 if istime else 0
                    except Exception as e:
                        self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}(istime类型)必须为0/1"})
                        self.error += 1
                elif typwant == "time":
                    try:
                        arg_ = float(args[func_pos].replace(" ",""))
                        if arg_ < 0:
                            self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}应大于0"})
                            self.error += 1
                        else:
                            args_[func_pos] = arg_
                    except Exception as e:
                        try:
                            ...
                            self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}不应为表达式"})
                            self.warning += 1
                            if arg_ < 0:
                                self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}应大于0"})
                                self.error += 1
                            ...
                            self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}无法解析"})
                            self.error += 1
                    if doubleType:
                    ...
                    try:
                        ...
                        if arg_ != int(arg_):
                            self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}应为整数"})
                            self.warning += 1
                        if arg_ < 0:
                            self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}应大于0"})
                            self.error += 1
                        ...
                        try:
                            ...
                            if not arg_default is None:
                                args_[func_pos] = arg_default
                                self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}无法解析"})
                                self.warning += 1
                            else:
                                self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}无法解析"})
                                self.error += 1
                    ...
                    if arg_[0] == "0" and len(arg_)>1:
                        if arg_[1] == "x":
                            try:
                                ...
                                self.log.append({"type": "error", "text": f"{arg_}不可解析"})
                                self.error += 1
                            continue
                        elif arg_[1]== "b":
                            try:
                    ...
                    if arg_ not in self.pnames:
                        if not arg_default is None:
                            args_[func_pos] = arg_default
                            self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}参数名[{arg_}]无法解析"})
                            self.warning += 1
                        else:
                            args_[func_pos] = arg_
                            self.log.append({"type": "error", "text": f"[{rule_origin}]输入{func_pos}参数名[{arg_}]无法解析"})
                            self.error += 1
                    else:
                        if func_name == "Para":
                            args_[func_pos] = arg_
                    ...
                    if arg_ not in self.fnames and func_name != "Fault":
                        if not arg_default is None:
                            args_[func_pos] = arg_default
                            self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}故障名[{arg_}]无法解析"})
                            self.warning += 1
                        else:
                            args_[func_pos] = arg_
                            self.log.append({"type": "warn", "text": f"[{rule_origin}]输入{func_pos}故障名[{arg_}]无法解析"})
        ...
        if "(" in rule or ")" in rule:
            for i in range(len(rule)):
                ...
                if rule[i + l] == "(":
                    ...
                    left -= 1
            if not cons:
                self.log.append({"type": "error", "text": f"[{self._backlog(rule,seqs)}] 引号不匹配"})
                self.error += 1
                rule += "\""
            if left < 0:
                self.log.append({"type": "error", "text": f"[{self._backlog(rule,seqs)}] 缺少{-left}个\"\(\""})
                self.error += 1
                rule = "("*(-left) + rule
            elif left > 0:
                self.log.append({"type": "error", "text": f"[{self._backlog(rule,seqs)}] 缺少{left}个\"\)\""})
    ...
    def _stringconvert(self,rule,seqs_):
        ...
        rule = rule_.replace("=","==").replace("====","==").replace("!==","!=").replace("<==","<=")\
               ...
               .replace("！","!")
        if rule != rule_:
            self.log.append({"type": "warn", "text": f"[{self._backlog(rule_, seqs)}]存在非法字符"})
            self.warning += 1
        if "," in rule:
            rule, rule_ = rule.split(",",1)
            self.log.append({"type": "error", "text": f"[{self._backlog(rule_, seqs)}]为多余部分"})
            self.error += 1
    ...
    def _parentheseconvert(self,rule,seqs_):
        ...
        while "(" in rule or ")" in rule:
            if "(" in rule and not ")" in rule:
                rule += ")"
                if not warnComplet:
                    self.log.append({"type": "error", "text": f"[{rule_origin}]缺少\"(\""})
    ...
    def _logicconvert(self, rule, seqs_):
        ...
        if not rule_split:
            while rule[:8]=="not not ":
                rule = rule[8:]
                if cons:
                    self.log.append({"type": "error", "text": f"[{seq_origin}]中存在无用not"})
                    self.warning += 1
                    cons = False
            ...
            while rule[-4:]==" not":
                rule = rule[-4:]
                if cons:
                    self.log.append({"type": "error", "text": f"[{seq_origin}]中存在未知not"})
                    self.error += 1
                    cons = False
            if not rule.replace(" ",""):
                rule = None
                self.log.append({"type": "error", "text": f"[{rule_origin}]中两逻辑关联词间缺少成分"})
                self.error += 1
            elif rule[:4]=="not ":
                rule_a, typ = self._compareconvert(rule_[4:],seqs)
                if typ not in ["bool","Unknown"]:
                    self.log.append({"type": "error", "text": f"[{seq_origin}] not后非布尔类型"})
                    self.error += 1
            ...
            for i in range(len_split):
                ...
                while rule_[:8]=="not not ":
                    rule_ = rule_[8:]
                    if cons:
                        self.log.append({"type": "warn", "text": f"[{seq_origin}]中存在无用not"})
                        self.warning += 1
                        cons = False
                ## Supprime wrong [not] in end(Case [not not xxx])
                if rule_[-1:] == " ":
                ...
                if not rule_.replace(" ",""):
                    ...
                    if typ not in ["bool","Unknown"]:
                        self.log.append({"type": "error", "text": f"[{seq_origin}] [{rule[a+1:b-1]}]后非布尔类型"})
                        self.error += 1
                    rule_ = rule_a
                if result:
                    ...
                    if not rule1.replace(" ",""):
                        ...
                        if typ not in ["bool","Unknown"]:
                            self.log.append({"type": "error", "text": f"[{seq_origin}]{rule[a+1:b-1]}前非布尔类型"})
                            self.error += 1
                        rule1 = rule_a
            ...
            for i in range(len_split):
                ...
                if not rule_:
                    ...
                    if typ not in ["num","Unknown"]:
                        self.log.append({"type": "error", "text": f"[{seq_origin}]{rule[a:b]}后非数值类型"})
                        self.error += 1
                    rule_ = rule_a
                if result:
                    ...
                    if not rule1:
                        ...
                        if typ not in ["num","Unknown"]:
                            self.log.append({"type": "error", "text": f"[{self._backlog(rule1,seqs)}]{rule[a:b]}前非数值类型"})
                            self.error += 1
                        rule1 = rule_a
    ...
    def _plusminusconvert(self,rule,seqs_):
        ...
        if "+-" in rule or "-+" in rule or "++" in rule:
            self.log.append({"type": "warn", "text": f"[{rule_origin}] 存在冗余正号"})
            self.warning += 1
        if "--" in rule:
            self.log.append({"type": "warn", "text": f"[{rule_origin}] 存在负号与减号的叠加"})
            self.warning += 1
        ...
        if not rule_split:
            ...
            rule1 = rule[a_:]
            if not rule1:
                self.log.append({"type": "error", "text": f"句末多余加减号"})
                self.error += 1
            else:
                rule_a, typ = self._proddivconvert(rule1,seqs)
                if typ not in ["num","Unknown"]:
    ...
    def _proddivconvert(self,rule,seqs_):
        ...
        if not rule_split:
            ...
            for i in range(len_split):
                ...
                if not rule_:
                    rule_ = None
                    self.log.append({"type": "error", "text": f"[{self._backlog(rule.replace('=','**'),seqs)}]中两乘除号间缺少成分"})
                    self.error += 1
                else:
                    rule_a, typ = self._powerconvert(rule_.replace("=","**"),seqs)
                    if typ not in ["num","Unknown"]:
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule_.replace('=','**'),seqs)}] {rule[a:b]}后非数值类型"})
                        self.error += 1
                    rule_ = rule_a
                ...
                if result:
                    ...
                    if not rule1.replace(" ",""):
                        rule1 = None
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule.replace('=','**'),seqs)}]中两乘除关联词间缺少成分"})
                        self.error += 1
                    else:
                        rule_a, typ = self._powerconvert(rule1.replace("=","**"),seqs)
                        if typ not in ["num","Unknown"]:
                            self.log.append({"type": "error", "text": f"[{self._backlog(rule1.replace('=','**'),seqs)}]{rule[a:b]}前非数值类型"})
                            self.error += 1
                        rule1 = rule_a
                    if func_name == "*":
                        result = ["/",["*",rule1,rule_]]
    ...
    def _powerconvert(self,rule,seqs_):
        ...
        result = []
        if "***" in rule:
            self.log.append({"type": "error", "text": f"[{self._backlog(rule[b:fin],seqs)}]中两(阶)乘号\"***\"间缺少成分"})
            self.error += 1
            rule = re.sub(r"\*\*\**", "**", rule)
        rule_split = list(re.finditer(r"\*\*", rule))
        if not rule_split:
            ...
            for i in range(len_split):
                ...
                if not rule_:
                    rule_ = None
                    self.log.append({"type": "error", "text": f"[{self._backlog(rule[b:fin],seqs)}]中两阶乘号\"**\"间缺少成分"})
                    self.error += 1
                else:
                    rule_a, typ = self._bitoperatorconvert(rule_,seqs)
                    if typ not in ["num","Unknown"]:
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule_,seqs)}]  **后为非数值类型"})
                        self.error += 1
                    rule_ = rule_a
                if result:
                    result = ["**",result,rule_]
                    ...
                    if not rule1.replace(" ",""):
                        rule1 = None
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule[:a],seqs)}]中两阶乘号\"**\"间缺少成分"})
                        self.error += 1
                    else:
                        rule_a, typ = self._bitoperatorconvert(rule1,seqs)
                        if typ not in ["num","Unknown"]:
                            self.log.append({"type": "error", "text": f"[{self._backlog(rule1,seqs)}]**前非数值类型"})
                            self.error += 1
                        rule1 = rule_a
                    result = ["**",rule1,rule_]
        return result, "num"
    ...
    def _bitoperatorconvert(self,rule,seqs_):
        ...
        if not rule:
            return None, "Unknown"
        if "~~" in rule:
            self.log.append({"type": "warn", "text": f"[{self._backlog(rule_,seqs)}]存在冗余~"})
            self.warning += 1
            while "~~" in rule:
                rule = rule.replace("~~", "")
        result = []
        ...
        if not rule_split:
            if rule[:1] == "~":
                ...
                rule_a, typ = self._bitoperatorconvert(rule_,seqs)
                if typ not in ["num","Unknown"]:
                    self.log.append({"type": "error", "text": f"[{self._backlog(rule_,seqs)}] {rule[a:b]}后非数值类型"})
                    self.error += 1
                return ["~", rule_a], "num"
            if rule[0] == "+":
                rule_ = rule[1:]
            ...
            if len(nums) > 2:
                self.log.append({"type": "error", "text": f"存在若干括号附近缺少运算符"})
                self.error += 1
                try:
                ...
                try:
                    ...
                    self.log.append({"type": "error", "text": f"{rule}不可解析"})
                    self.error += 1
                    return None, "Unknown"
            ...
            for i in range(len_split):
                ...
                if not rule_:
                    rule_ = None
                    self.log.append({"type": "error", "text": f"[{self._backlog(rule[b:fin],seqs)}]中两位操作间缺少成分"})
                    self.error += 1
                else:
                    rule_a, typ = self._bitoperatorconvert(rule_,seqs)
                    if typ not in ["num","Unknown"]:
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule[a:b],seqs)}] {rule[a:b]}后为非数值类型"})
                        self.error += 1
                    rule_ = rule_a
                func_name = rule[a:b]
                if result:
                    ...
                    if not rule1:
                        rule1 = None
                        self.log.append({"type": "error", "text": f"[{self._backlog(rule[:a+10],seqs)}]中两位操作间缺少成分"})
                        self.error += 1
                    else:
                        rule_a, typ = self._bitoperatorconvert(rule1,seqs)
                        if typ not in ["num","Unknown"]:
                            self.log.append({"type": "error", "text": f"[{self._backlog(rule1,seqs)}] {rule[a:b]}后非数值类型"})
                            self.error += 1
                        rule1 = rule_a
                    result = [func_name, rule1, rule_]
        return result, "num"
