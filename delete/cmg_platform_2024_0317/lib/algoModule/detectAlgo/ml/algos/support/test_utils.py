...
def _auc_calculate(preLabel, realLabel, n_bins=100):
    ...
    #print(len(preLabel), len(realLabel))
    ...
    for rv, pv in zip(realLabel, preLabel):
...
def _inner_test_process(mdlClass, trainPath, testPath, pnames=None, nlimit=5, nstep=1, dir_="./Figure"):
    """[面向性能评估] 数据集上的算法性能评估
    [Inputs]
    `mdlClass: Object, 算法类
    ...
    """
    try:
        train_data = pd.read_csv(trainPath, index_col=0, encoding="gbk", na_values=['--']).fillna(method="ffill", limit=nlimit).dropna(how="any", axis=0)
    except:
        train_data = pd.read_csv(trainPath, index_col=0, na_values=['--']).fillna(method="ffill", limit=nlimit).dropna(how="any", axis=0)
    if pnames is None:
    ...
    model.fit(np.array(train_data), save=False)
    
    try:
        test_data = pd.read_csv(testPath, index_col=0, encoding="gbk", na_values=['--']).fillna(method="ffill", limit=nlimit).dropna(how="any", axis=0)
    except:
        test_data = pd.read_csv(testPath, index_col=0, na_values=['--']).fillna(method="ffill", limit=nlimit).dropna(how="any", axis=0)
    try:
    ...
    ablist, scores, suppInfo = model.validate(np.array(test_data))

    if not dir_ is None:
        datasetname = trainPath.split("\\")[-2]  #将训练集路径逐个分割，取倒数第二个如D:\testCaseData\ALFA\data_test.csv,则为ALFA
        if not os.path.exists(dir_):
            os.mkdir(dir_)
        if not os.path.exists(os.path.join(dir_, datasetname)):
            os.mkdir(os.path.join(dir_, datasetname))
        ...
        for pid, pn in enumerate(pnames):      #将pnames整为迭代对象,pid为pnames中的每个元素编号，test_data[:, pid]取出第pid列的所有元素
            ...
            if np.ndim(scores) == 2:
                ablist_ = np.where(scores[:, pid] > 0.5)[0]
                plt.scatter(ablist_, test_data[ablist_, pid], marker="o", c="r", label="异常帧")
            else:
            ...
            plt.savefig(os.path.join(dir_, datasetname, f"{pn}.png"))
            plt.close()
        plt.figure(figsize=(12,10))
        #plt.fill_between(np.arange(len(realLabel)), 0, 1,
        #                 where=(realLabel>=0.5), color="r", alpha=0.4)
        if np.ndim(scores) == 2:
            scores = np.max(scores, axis=-1)
        plt.plot(scores, "b", linewidth=1., label="异常分数")
        plt.tight_layout()      #自动调整图形的布局
        plt.savefig(os.path.join(dir_, datasetname, "anom.png"))
        plt.close()
...
def _inner_get_rule(mdlClass, trainPath, testPath, pnames=None, nlimit=5, nstep=1):
    ...
    test_data = test_data.loc[::nstep, pnames].values.astype("float32")
    return {
        "null_rule": model.rule_induce(np.array(train_data)),
        "rule": model.rule_induce(np.array(test_data))
        }

def test_process(mdlClass, dataPathMap, nlimit=5, nstep=1, dir_="./Figure"):
    """[面向性能评估] 数据集上的算法性能评估
    [Inputs]
    `mdlClass: Object, 算法类
    `trainPath: str, 训练数据集路径
    ...
    for dId, dataConfig in enumerate(dataPathMap):
        ...
        res[dataDir.split("\\")[-1] + "(" + "/".join(pnames or ["all"]) + ")"] = _inner_test_process(mdlClass, trainPath, testPath,
                                                                                                     pnames=pnames, nlimit=nlimit, nstep=nstep_)
    return res
...
def get_rule(mdlClass, dataPathMap, nlimit=5, nstep=1):
    ...
    for dId, dataConfig in enumerate(dataPathMap):
        ...
        trainPath = os.path.join(dataDir, "data.csv")
        testPath = os.path.join(dataDir, "data_test.csv")
        res[dataDir.split("\\")[-1] + "(" + "/".join(pnames or ["all"]) + ")"] = _inner_get_rule(mdlClass, trainPath, testPath,
                                                                                                 pnames=pnames, nlimit=nlimit, nstep=nstep_)
    return res
