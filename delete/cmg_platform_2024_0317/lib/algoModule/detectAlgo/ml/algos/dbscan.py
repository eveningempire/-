...
def _get_rule(nonvalid, valid_inds, pnames, tot_corrIds, tot_reference, thred, eps=1e-5):
    n = int(round(-np.log(eps)/np.log(10)))
    rule = []
    for itm in nonvalid:
        pid_cid, cond_pid = itm
        pid, cid = pid_cid
        p_true_id = valid_inds[pid]
...
class Model(BaseModel): # Use Model Template
    ...
    def fit(self, data, save=False):
        ...
        with warnings.catch_warnings():
            ...
            for _ in range(3):
                err[np.abs(err)<np.percentile(np.abs(err),99,axis=0)/10] = 0
                #err[np.abs(err)<np.max(np.abs(err))/10] = 0
            eps_neighors = 10 * np.mean(np.maximum(err, self.min_eps)**2, axis=0)**0.5
            ...
            self.valid_inds, self.tot_corrIds, self.tot_reference = self._getRef(self.cluster_mins, self.cluster_maxs,
    ...
    def _single_validate(self, V, inner=False):
        ...
        for i, p in enumerate(self.valid_inds):
        ...
        score = np.minimum(1, np.maximum(0, np.maximum(subD-V, V-supD)/(self.thres*2)))
    ...
    def from_json(self, configJson=b""):
        ...
        self.valid_inds = config["valid_inds"]
    ...
    def rule_induce(self, data, beta=0.75, eps=1e-5):
        ...
        return _get_rule(nonvalid, self.valid_inds, self.pnames, self.tot_corrIds, tot_reference, self.thres, eps=eps)
...
if __name__ == "__main__":
    ...
    pprint(test_process(Model, [
        ...
        [os.path.join(DIR, 'simulatedDataSet'),['左机轮刹车给定','左机轮刹车液压压力']]
        ], nstep=1))
