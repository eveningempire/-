        ...
        try:
            ...
            if  (not trigTime is None and lastTime < trigTime) or \
               data["actualTime"] - lastTime > t:
                ...
                return True, min((data["actualTime"] - lastFalseTime)/t/2, 1)
            else:
                return False, (data["actualTime"] - lastFalseTime)/t/2
        except Exception as e:
            return None, 0
        ...
        try:
            ...
            if (not trigTime is None and lastTime < trigTime) or \
               data["actualTime"] - lastTime > t:
                ...
                return True, min((data["actualTime"] - lastTrueTime)/t/2, 1)
            else:
                return False, t/(data["actualTime"] - lastTrueTime)/2
        except Exception as e:
            return None, 0
    ...
    def _partialcond(data, args, trigTime=None):
        try:
            ...
            if len(value_) < n:
                ...
                if trueCount >= n:
                    return True, sorted(score_[startId:])[-m]
                else:
        ...
        try:
            ...
            if args[2]: ## istime = 1 => Time Mode
                ...
                if len(creaseData["time"]) < 2 or creaseData["time"][-2] <= splitTime:
                    return None, 0
                elif creaseData["lastDecTime"] is None:
                    return True, 1
                elif creaseData["lastDecTime"] <= splitTime:
                    return True, min(1, (data["actualTime"] - creaseData["lastDecTime"])/args[1]/2)
                else:
                    return False, (data["actualTime"] - creaseData["lastDecTime"])/args[1]/2
            else: ## istime = 0 => Frame Mode
                value_ = creaseData["value"][-args[1]+1:]
                allCount = len(value_)
                if trigTime is None:
                    trueCount = sum([d_next > d_pre for d_pre, d_next in zip(value_[:-1], value_[1:])])
                    if allCount > 1:
                        scores = [score_ge(d_next, d_pre) for d_pre, d_next in zip(value_[:-1], value_[1:])]
                        trueCount = sum([s >= 0.5 for s in scores])
                        return trueCount >= allCount, min(scores)
                    else:
                        return None, 0
                else:
                    ...
                    if allCount > 1:
                        scores = [score_ge(d_next, d_pre) for d_pre, d_next in zip(value_[:-1], value_[1:])]
                        trueCount = sum([s >= 0.5 for s in scores]); allCount = len(value_)
                        return trueCount >= allCount, min(scores)
                    else:
                        return None, 0
        except Exception as e:
    ...
    def Decrease(data, args, trigTime=None):
        try:
            ...
            if args[2]: ## istime = 1 => Time Mode
                ...
                if len(creaseData["time"]) < 2 or creaseData["time"][-2] <= splitTime:
                    return None, 0
                elif creaseData["lastIncTime"] is None:
                    return True, 1
                elif creaseData["lastIncTime"] <= splitTime:
                    return True, min(1, (data["actualTime"] - creaseData["lastIncTime"])/args[1]/2)
                else:
                    return False, (data["actualTime"] - creaseData["lastIncTime"])/args[1]/2
            else: ## istime = 0 => Frame Mode
                value_ = creaseData["value"][-args[1]+1:]
                allCount = len(value_)
                if trigTime is None:
                    if allCount > 1:
                        scores = [score_ge(d_pre, d_next) for d_pre, d_next in zip(value_[:-1], value_[1:])]
                        trueCount = sum([s >= 0.5 for s in scores])
                        return trueCount >= allCount, min(scores)
                    else:
                        return None, 0
        ...
        try:
            ...
            if trigTime_ == -1:
                ...
                if trigTime_ + args[1] <= data["actualTime"]:
                    return True, min(1, ( data["actualTime"] - trigTime_) / args[1]/2)
                else:
                    return False, ( data["actualTime"] - trigTime_) / args[1]/2
            else:
                return None, 0
        except Exception as e:
            return None, 0
